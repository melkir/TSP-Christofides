...
bool is_path_shorter(int **graph, int v1, int v2, int v3, int v4, int &total_dist) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i + 2; (j + 1) % n != i; j++) {
            ...
            if (is_path_shorter(graph, path[u1], path[v1], path[u2],
                ...
                if (pathLength - old_distance < 5 && counter == term_cond) break;
                // reset i
                if (pathLength - old_distance > term_cond) i = 0;
