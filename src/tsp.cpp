...
void TSP::getNodeCount() {
    ...
    std::string unused;
    while (std::getline(inStream, unused)) ++count;
    n = count;
    inStream.close();
};
...
int TSP::get_distance(struct TSP::City c1, struct TSP::City c2) {
    int dx = (int) pow((float) (c1.x - c2.x), 2);
    int dy = (int) pow((float) (c1.y - c2.y), 2);
    return (int) (floor((float) (sqrt(dx + dy)) + 0.5));
};
...
void *F(void *args) {
    ...
    TSP *tsp = my_data->tsp;
    int **graph = tsp->graph;
    int start = tsp->start_idx[tid];
    int end = tsp->end_idx[tid];
...
void TSP::findMST() {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int u = 0; u < n; u++) {
            if (graph[v][u] && !in_mst[u] && graph[v][u] < key[u]) {
...
void TSP::findOdds() {
    ...
    for (int r = 0; r < n; r++) {
        if ((adjlist[r].size() % 2) != 0) odds.push_back(r);
    }
}
...
void TSP::make_hamilton(vector<int> &path, int &path_dist) {
    ...
    while (next != path.end()) {
        ...
        if (!visited[*next]) {
            ...
            visited[*curr] = true;
            next = curr + 1;
        } else next = path.erase(next); // remove it
    }
...
void TSP::create_tour(int pos) {
    ...
    euler(pos, circuit);
    // make it hamiltonian and pass actual vars
